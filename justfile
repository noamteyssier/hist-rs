...
bench:
    hyperfine \
        ...
        -n "naive-no-cache" "export LC_ALL=C; sort < {{input_file}} | uniq -c | sort -n >{{output_file}}" \
        -n "naive-no-cache-size-hints" "export LC_ALL=C; sort < {{input_file}} -S 1G | uniq -c | sort -S 1G -n >{{output_file}}" \

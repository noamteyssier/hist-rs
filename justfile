...
bench:
    hyperfine \
        ...
        -n "naive-no-cache" "LC_ALL=C; sort < {{input_file}} | uniq -c | sort -n >{{output_file}}" \
        -n "naive-no-cache-size-hints" "LC_ALL=C; sort < {{input_file}} -S 1G | uniq -c | sort -S 1G -n >{{output_file}}" \
        -n "naive" "sort < {{input_file}} | uniq -c | sort -n >{{output_file}}"
